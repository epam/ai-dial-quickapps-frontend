          ...
          >
            {selectedItems &&
              selectedItems.map((selectedItemForRender, index) => {
                ...
                return (
                  <Fragment
                    key={`selected-item-${getItemLabel(selectedItemForRender)}-${index}`}
                  >
                    {showItemsConnector && (
                      <span
                    ...
                    >
                      ...
                      >
                        ...
                        <CloseButtonSmall
                          data-qa={`unselect-item-${getItemValue(selectedItemForRender)}`}
                          disabled={disabled}
                          onClick={(e) => {
                            e.stopPropagation();
                            e.preventDefault();
          ...
          >
            {displayedItems?.length > 0
              ? displayedItems.map((item, index) => (
                  ...
                  >
                    {itemRow ? createElement(itemRow, { item }) : getItemLabel(item)}
                  </li>
                ))
              : !!inputValue?.length && (
                  <li className="px-3 py-2">
                    {notFoundPlaceholder || 'No available items'}
                  </li>
                )}
          </ul>
        </div>
